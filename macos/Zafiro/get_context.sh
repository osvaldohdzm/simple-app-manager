...
EXCLUDE_DIRS=(
    ...
    android/.gradle android/app/build ios/build ios/Pods ios/Runner.xcworkspace
    linux/build macos/build windows/build web/build
    linux/flutter/ephemeral macos/Flutter/ephemeral windows/flutter/ephemeral
    "*.xcassets"
)
# Archivos a excluir por nombre/patrón
EXCLUDE_FILES=(
    "Podfile.lock" "package-lock.json" "yarn.lock" "*.lock" "macos/Podfile"
    "*.iml" ".DS_Store" "local.properties" "*.pbxuser" "*.xcscheme"
    "gradlew" "gradlew.bat" ".metadata" ".flutter-plugins" ".flutter-plugins-dependencies"
    "flutter_export_environment.sh" "generated_plugins.cmake"
    "GeneratedPluginRegistrant.java" "GeneratedPluginRegistrant.h" "GeneratedPluginRegistrant.m"
    "GeneratedPluginRegistrant.swift" "GeneratedPluginRegistrant.cc"
    "*.xcworkspace" "*.appiconset" "*.imageset" "code-context.txt" "LICENSE" "README.md"
)
# Extensiones de archivo a excluir
EXCLUDE_EXTENSIONS=(
    png jpg jpeg gif webp ico svg ttf otf zip tar gz a so jar dll bin apk aar ipa app exe
    db db-journal prof profm textproto storyboard xib arsc flat d dex dic xcconfig plist entitlements pbxproj xcworkspacedata xcsettings
    cmake gradle properties cjs mjs d.ts d.mts yaml yml json md txt sh py h gitignore cc cpp rc manifest js swift
)
