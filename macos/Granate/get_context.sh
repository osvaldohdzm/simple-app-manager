...
# --- Exclusiones específicas para Flutter ---
EXCLUDE_DIRS=(
  .git .idea .vscode build .dart_tool android/.gradle android/app/build ios/build ios/Pods ios/Runner.xcworkspace
  linux/build macos/build windows/build web/build linux/flutter/ephemeral macos/Flutter/ephemeral windows/flutter/ephemeral
  "*.xcassets"
)
EXCLUDE_FILES=(
  "Podfile.lock" "package-lock.json" "yarn.lock" "*.lock" "macos/Podfile" "*.iml" ".DS_Store" "local.properties"
  "*.pbxuser" "*.xcscheme" "gradlew" "gradlew.bat" ".metadata" ".flutter-plugins" ".flutter-plugins-dependencies"
  "flutter_export_environment.sh" "generated_plugins.cmake" "GeneratedPluginRegistrant.java" "GeneratedPluginRegistrant.h"
  "GeneratedPluginRegistrant.m" "GeneratedPluginRegistrant.swift" "GeneratedPluginRegistrant.cc" "*.xcworkspace"
  "*.appiconset" "*.imageset" "code-context.txt" "LICENSE" "README.md"
)
EXCLUDE_EXTENSIONS=(
  png jpg jpeg gif webp ico svg ttf otf zip tar gz a so jar dll bin apk aar ipa app exe db db-journal prof profm textproto
  storyboard xib arsc flat d dex dic xcconfig plist entitlements pbxproj xcworkspacedata xcsettings cmake gradle properties
  cjs mjs d.ts d.mts yaml yml json md txt sh py h gitignore cc cpp rc manifest js swift
)

# --- Solo incluir archivos fuente relevantes para Flutter ---
INCLUDE_EXTENSIONS=(dart yaml)

# --- Construcción de argumentos para 'find' ---
prune_args=()
...
echo "Generando contexto de código fuente relevante para Flutter..."
...
find "$PROJECT_DIR" \
  ...
  ! \( "${name_args[@]}" \) \
  ! \( "${ext_args[@]}" \) \
  \( -name "*.dart" -o -name "pubspec.yaml" -o -name "analysis_options.yaml" \) \
  -print0 |
...
echo "¡Éxito! El contexto relevante de Flutter se ha escrito en $OUTPUT_FILE"
